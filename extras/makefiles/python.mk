...
${VIRTUALENV_CMD}:
	@test -d ${VIRTUALENV_DIR} || python3 -m virtualenv ${VIRTUALENV_ARGS} ${VIRTUALENV_DIR} > /dev/null && touch $@
...
python_egg: setup.py ${PYTHON_COMPILED}
	${VIRTUALENV} python3 setup.py bdist_egg --exclude-source-files > /dev/null

python_wheel: setup.py ${PYTHON_COMPILED} ${CHECKPOINT_DIR}/wheel.check
	${VIRTUALENV} python3 setup.py bdist_wheel > /dev/null
